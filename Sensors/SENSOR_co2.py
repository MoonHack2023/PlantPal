...
class CO2_SENSOR():
    ...
    def on_device_start(self):
        if self.fw_mode != 0:
            ...
            print("co2: Transfer from boot to application")
            self.start()
            time.sleep(0.5)
            status = self.get_status()
    ...
    def get_data(self):
        ...
        if data_av == "1":
            print("DATA AVAILABILE CO2 SENSOR")
    ...
    def set_conditions(self,temp,hum):
        # formula specified in datasheet 
        # Humidity is stored as an unsigned 16 bits in 1/512%RH (so need to multiply by 512)
        # default value is 50% = 0x64, 0x00
        print("SETTING ENVIRONMENT DATA FOR C02 SENSOR")
        hum = int(hum*512)
        hum_str = format(hum, '016b')
        high_hum = int(hum_str[:8],2) # write this to element 0
        low_hum  = int(hum_str[8:],2) # write this to element 1

        #Temperature is stored as an unsigned 16 bits integer in 1/512 degrees
        # The default value is 25°C = 0x64, 0x00
        temp = temp - 25
        temp = int(temp*512)
        temp_str = format(temp, '016b')
        high_temp = int(temp_str[:8],2) # write this to element 2
        low_temp  = int(temp_str[8:],2) # write this to element 3

        self.bus.write_i2c_block_data(self.ADDR, self.ENV_DATA, [high_hum,low_hum,high_temp,low_temp])
        time.sleep(1)
